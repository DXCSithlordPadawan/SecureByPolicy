...
| RCT-011 | `.innerHTML =` (non-empty assignment) | High | OWASP A03 / DISA STIG: Assigning to `.innerHTML` with unsanitized data bypasses React's built-in XSS protection and can inject malicious scripts. | Use `element.textContent` for plain text, or sanitize with `DOMPurify.sanitize()` before assigning to `innerHTML`. Prefer React-controlled rendering via JSX and component state. |
| RCT-012 | `target="_blank"` without `rel="noopener noreferrer"` (both values required) | Medium | OWASP A05 / CIS Level 2 (react/jsx-no-target-blank): Omitting `rel="noopener noreferrer"` on `target="_blank"` links exposes `window.opener` and enables tabnapping attacks. | Add `rel="noopener noreferrer"` to every element that uses `target="_blank"`. |
| RCT-013 | `apiKey`, `api_key`, `secretKey`, `secret_key`, `accessToken`, `access_token`, or `authToken` assigned a hardcoded string literal | High | OWASP A07 / NIST IA-5: Hardcoded API key or authentication token detected in source code. Secrets committed to source control are exposed in repositories and build artifacts. | Remove all hardcoded credentials. Load API keys and tokens from environment variables (`process.env.REACT_APP_API_KEY`) or a secrets manager at runtime. |
| RCT-014 | `'use server'` directive (flags all occurrences for mandatory review — verify Zod or Yup schema validation is present in the same function) | Critical | CVE-2025-55182 / OWASP A03: React 19 Server Actions without input validation are vulnerable to injection attacks. Unvalidated `formData` inputs can pass malicious data directly into server-side operations. Every `'use server'` function must be reviewed to confirm schema validation is in place. | Validate all Server Action inputs using a schema validation library (Zod `z.object({...}).parse(...)` or Yup) before use. See the React 19 Security Features section of the guide. Mark as reviewed once validation is confirmed. |
| RCT-015 | `devTools: true` (Redux store configured with DevTools unconditionally enabled) | Medium | OWASP A02 / CIS Level 2: Enabling Redux DevTools unconditionally in production exposes the full application state tree — including potentially sensitive data — to browser extensions and developer tools. | Guard the flag with an environment check: `devTools: process.env.NODE_ENV !== 'production'`. |
