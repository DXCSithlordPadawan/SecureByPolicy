...
| TS-012 | `document.write(` | High | OWASP A03: `document.write()` with dynamic content enables XSS attacks by injecting arbitrary HTML into the page. | Remove `document.write()`. Use DOM manipulation methods (`createElement`, `appendChild`, `textContent`) instead. |
| TS-013 | `dangerouslySetInnerHTML=` | High | OWASP A03: Using `dangerouslySetInnerHTML` in React with unsanitized data enables XSS attacks. | Sanitize HTML with `DOMPurify.sanitize()` before passing to `dangerouslySetInnerHTML`, or use a `SafeHTML` branded type wrapper as described in the TypeScript Security Best Practices Guide. |
| TS-014 | `createHash('sha1')` / `createHash("sha1")` / `createHash('md4')` / `digest('SHA-1')` | High | FIPS 140-3 / NIST SP 800-53 SC-13: SHA-1, MD4, and MD2 are not in the FIPS 140-3 approved algorithm list and must not be used for cryptographic operations. | Replace with a FIPS 140-3 approved algorithm: `createHash('sha256')` (Node.js) or `crypto.subtle.digest('SHA-256', data)` (Web Crypto API). |
| TS-015 | `api_?key`, `apiKey`, `auth_?token`, or `secret` assigned a hardcoded string literal | High | OWASP A07 / NIST IA-5: Hardcoded API key, authentication token, or secret detected in source code. Secrets in source code are exposed in version control and build artifacts. | Remove hardcoded secrets. Load all credentials and API keys from environment variables (`process.env.API_KEY`) or a dedicated secrets manager. |
